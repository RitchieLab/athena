...
// 		restrictStepsDone=0;
		...
		GA2DBinaryStringGenome caseGenome(totalVars,totalVars, GAFunct::GACaseObjective);
		GA2DBinaryStringGenome controlGenome(totalVars,totalVars, GAFunct::GAControlObjective);
...
void GADiscrimBayes::totalModels(GASimpleGA* ga,  map<vector<vector<int> >, ModelScores>& topModels,
	...
	for(int i=0; i<nGenomes; i++){
		GA2DBinaryStringGenome genome = (GA2DBinaryStringGenome&)ga->population().best(i);
...
///
vector<vector<int> > GADiscrimBayes::constructEquation(GA2DBinaryStringGenome& genome){
	vector<int> empty;
