...
void NNLog::output_log(ostream& os){
  ...
  // output headers
  os << setw(5) << left << "Gen" << setw(11) << "ValidNN" << setw(10) << "AvgSize" << setw(8) << "AvgFit" 
  ...
  for(unsigned int gen=0; gen < gens.size(); gen++){
    os << setw(5) << left << gen << setw(11) << gens[gen].numValidNN << setw(10) << gens[gen].avgSize << setw(8) << gens[gen].avgFitness 
...
void NNLog::output_fitness(std::ostream& os, unsigned int totalPopSize){
  ...
  void NNLog::sendLog(){
    ...
    for(unsigned int i=0; i< gens.size(); i++){
      total_size += 11; // 10 info + 1 for size of best model
