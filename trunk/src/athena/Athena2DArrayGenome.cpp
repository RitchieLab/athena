#ifndef _athena_array2_cpp_
#define _athena_array2_cpp_
...
Athena2DArrayGenome<T>::className() const {return "Athena2DArrayGenome";}
template <class T> int
Athena2DArrayGenome<T>::classID() const {return GAID::ArrayGenome2D;}

template <class T>
Athena2DArrayGenome<T>::
Athena2DArrayGenome(unsigned int width, unsigned int height,
...
template <class T>
Athena2DArrayGenome<T>::
Athena2DArrayGenome(const Athena2DArrayGenome<T> & orig) : GAArray<T>(orig.sz){
  Athena2DArrayGenome<T>::copy(orig);
}


template <class T>
Athena2DArrayGenome<T>::~Athena2DArrayGenome() { }


template <class T> void
Athena2DArrayGenome<T>::copy(const GAGenome & orig){
  if(&orig == this) return;
  const Athena2DArrayGenome<T>* c = DYN_CAST(const Athena2DArrayGenome<T>*, &orig);
  if(c) {
    GAGenome::copy(*c);
    GAArray<T>::copy(*c);
    nx = c->nx; ny = c->ny;
...
Athena2DArrayGenome<T>::clone(GAGenome::CloneMethod flag) const {
  Athena2DArrayGenome<T> *cpy = new Athena2DArrayGenome<T>(nx,ny);
  if(flag == CONTENTS){
    cpy->copy(*this);
  }
  else{
...
template <class T> int
Athena2DArrayGenome<T>::resize(int w, int h){
...
template <class T> int
Athena2DArrayGenome<T>::read(STD_ISTREAM &) {
...
template <class T> int
Athena2DArrayGenome<T>::write(STD_OSTREAM & os) const
{
...
template <class T> int
Athena2DArrayGenome<T>::resizeBehaviour(GAGenome::Dimension which) const {
  int val = 0;
  if(which == WIDTH) {
		...
		unsigned int lower, unsigned int upper)
{
  ...
  case WIDTH:
    ...
    if(nx > upper) Athena2DArrayGenome<T>::resize(upper,ny);
    if(nx < lower) Athena2DArrayGenome<T>::resize(lower,ny);
    break;

  case HEIGHT:
    minY = lower; maxY = upper;
    if(ny > upper) Athena2DArrayGenome<T>::resize(nx,upper);
    if(ny < lower) Athena2DArrayGenome<T>::resize(nx,lower);
    break;
...
template <class T> void
Athena2DArrayGenome<T>::copy(const Athena2DArrayGenome<T> & orig,
...
template <class T> int
Athena2DArrayGenome<T>::equal(const GAGenome & c) const
{
  if(this == &c) return 1;
  Athena2DArrayGenome<T> & b = (Athena2DArrayGenome<T> &)c;
...
Athena2DArrayGenome<ARRAY_TYPE>::SwapMutator(GAGenome & c, float pmut)
{
  Athena2DArrayGenome<ARRAY_TYPE> &child=DYN_CAST(Athena2DArrayGenome<ARRAY_TYPE>&, c);
  register int n, i;
...
Athena2DArrayGenome<ARRAY_TYPE>::
ElementComparator(const GAGenome& a, const GAGenome& b)
{
  const Athena2DArrayGenome<ARRAY_TYPE>& sis=
    DYN_CAST(const Athena2DArrayGenome<ARRAY_TYPE>&, a);
  const Athena2DArrayGenome<ARRAY_TYPE>& bro=
    DYN_CAST(const Athena2DArrayGenome<ARRAY_TYPE>&, b);
...
UniformCrossover(const GAGenome& p1, const GAGenome& p2,
  ...
  const Athena2DArrayGenome<T> &mom=DYN_CAST(const Athena2DArrayGenome<T> &, p1);
  const Athena2DArrayGenome<T> &dad=DYN_CAST(const Athena2DArrayGenome<T> &, p2);

  int nc=0;
  int i,j;

  if(c1 && c2){
    Athena2DArrayGenome<T> &sis=DYN_CAST(Athena2DArrayGenome<T> &, *c1);
    Athena2DArrayGenome<T> &bro=DYN_CAST(Athena2DArrayGenome<T> &, *c2);
