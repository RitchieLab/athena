...
/// @param base base portion of output file name
///
void OutputManager::outputIndCheckPt(std::stringstream &ss, int currCV){
	string outputName = basename + "." + Stringmanip::numberToString(currCV+1) + ".indchk.txt";
	ofstream chkPoint(outputName.c_str(), ios::out);
	chkPoint << ss.str();
	chkPoint.close();
}

///
/// read individual output checkpoint files
///
void OutputManager::readIndCheckPts(std::stringstream &ss, int totalCV){
	for(int i=0; i<totalCV; i++){
		string outputName = basename + "." + Stringmanip::numberToString(i+1) + ".indchk.txt";
		ifstream chkPoint(outputName.c_str());
		ss << chkPoint.rdbuf();
		chkPoint.close();
