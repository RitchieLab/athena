...
// Evaluates genome and resets stats
// added by smd 10-09
void GASimpleGA::evaluatePop(){
  pop->evaluate(gaTrue);
  stats.reset(*pop);
}
...
//   The selection routine must return a pointer to a genome from the old
// population.
// Added this routine for use in prune and plant only
void
GASimpleGA::prune_and_plant_step()
{
  int i, mut, c1, c2;
  GAGenome *mom, *dad;          // tmp holders for selected genomes
...
// the main population.
  ...
  for(i=0; i<pop->size()-1; i+=inc){	// takes care of odd population
    mom = &(oldPop->select());
    dad = &(oldPop->select());
    stats.numsel += 2;		// keep track of number of selections
    ...
    // need to check for prune and plant operation
    for(int j=0; j<2; j++){
      if(GAFlipCoin(getPrunePlant()) && i+j+2 < pop->size() ){
        inc += (*pandp)(&pop->individual(i+j), &pop->individual(i+j+2));
      }
    }
    totaladded += inc;
  }
  ...
  //if(pop->size() % 2 != 0){	// do the remaining population member
  // do any remaining population members
  while(totaladded < pop->size()){
    mom = &(oldPop->select());  
    dad = &(oldPop->select());
    stats.numsel += 2;		// keep track of number of selections
