...
public:
  ...
  typedef int   (*PruneAndPlant)(GAGenome*, GAGenome*);
...
public:
  ...
  float altscore() const {return _altscore;}
  float altscore(float s){return _altscore=s;}
  ...
  /// added smd 1/13/09 to allow user to force an evaluation at next appropriate time
  void setEvaluated(GABoolean status){_evaluated=status;}
...
//  unsigned int effSize;
  ...
  float _altscore;  // added for use in double tournament selection
